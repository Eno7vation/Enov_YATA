...
> ```docker exec -it enov_yata/django /bin/bash``` <br/>

> ```yum install update``` <br/>
...
Eno789님은 PM, Devops(Golang, Python), Frontend(JS/DTL), Backend(Utils)를 담당했습니다.
...
DB 가이드
-------------
#### 기본적으로 DB는 제공되므로, 오류가 생길시에만 아래의 프로세스를 진행해주세요.
#### 실행가이드 1번을 진행해주세요.
...
[접속 링크](http://127.0.0.1:8000) -> http://127.0.0.1:8000

해당 소스코드 관련 문의 -> eno7contact@gmail.com(미포함 소스코드는 제공하지 않습니다.)
